...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl xz-utils
RUN mkdir ~/qt-source && curl http://mirrors.ocf.berkeley.edu/qt/archive/qt/5.11/5.11.1/single/qt-everywhere-src-5.11.1.tar.xz -O ~/qt-source/qt-everywhere-src-5.11.1.tar.xz && md5sum ~/qt-source/qt-everywhere-src-5.11.1.tar.xz && cd ~/qt-source && tar xvf qt-everywhere-src-5.11.1.tar.xz
...
# RUN mkdir ~/qt-build && cd ~/qt-build && ~/qt-source/configure -release -nomake tests -nomake examples -confirm-license -prefix $PWD/qtbase -opensource
