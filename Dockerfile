...
RUN mkdir ~/qt-source && wget --referer=http://download.qt.io http://download.qt.io/official_releases/qt/5.11/5.11.1/single/qt-everywhere-src-5.11.1.tar.xz -o ~/qt-source/qt-everywhere-src-5.11.1.tar.xz && md5sum ~/qt-source/qt-everywhere-src-5.11.1.tar.xz && cd ~/qt-source && tar -x -f qt-everywhere-src-5.11.1.tar.xz
...
# RUN mkdir ~/qt-build && cd ~/qt-build && ~/qt-source/configure -release -nomake tests -nomake examples -confirm-license -prefix /opt/qtbase -opensource
