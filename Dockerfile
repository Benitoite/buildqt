...
RUN apt-key update && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl xz-utils

RUN mkdir ~/qt-inst && cd ~/qt-inst && curl -i 'http://mirrors.ocf.berkeley.edu/qt/archive/qt/5.11/5.11.1/single/qt-everywhere-src-5.11.1.tar.xz' -O qt-everywhere-src-5.11.1.tar.xz && md5sum qt-everywhere-src-5.11.1.tar.xz

RUN mkdir ~/qt-source && cd ~/qt-source && tar xvf ~/qt-inst/qt-everywhere-src-5.11.1.tar.xz

RUN mv ~/qt-source/qt-everywhere-src-5.11.1 ~/qt-source/qt
...
# RUN mkdir ~/qt-build && cd ~/qt-build && ~/qt-source/configure -release -nomake tests -nomake examples -confirm-license -prefix /opt/local/Qt -opensource
